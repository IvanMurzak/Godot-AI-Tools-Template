...
| --- | --- | --- |
| `test.yml` | `workflow_call` (reusable) | **Leg 1 (deterministic, no Godot binary):** build the source-only package, run pure-managed unit tests, `dotnet pack`, assert the nupkg is source-only with no GodotSharp dep, then build the **testbed consuming the local package** (proves the source-injection recipe). **Leg 2 (live):** install the core addon into the testbed, then build + boot a real headless Godot via the **safe three-process pattern** — `--import` (materialize `.godot/`) → `dotnet build` the testbed csproj → `--editor --quit` (assert a clean boot). It deliberately does NOT use cold `--build-solutions --quit`, which SIGSEGVs on a first C# `[Tool]` EditorPlugin (upstream godot #81790/#100805). Calling each tool via `godot-cli run-tool` is a separate non-failing TODO (finish wiring the local server). |
